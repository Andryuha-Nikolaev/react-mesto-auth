...
function PopupWithForm({ name, title, children, buttonText, isOpen, onClose, onSubmit, onLoading }) {
  return (
    <div className={`popup popup_type_${name} ${isOpen ? "popup_opened" : ""}`} id="popup-edit">
      <div className="popup__container">
        ...
        <form name={name} className="form" noValidate onSubmit={onSubmit}>
          ...
          {children}
          <button type="submit" className={`form__button-save ${onLoading ? "form__button-save_loading" : ""}`}>
            {buttonText}
          </button>
        </form>
      </div>
